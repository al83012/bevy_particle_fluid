...
use bevy::color::palettes::tailwind::{BLUE_200, ORANGE_400, RED_500};
use bevy::math::Vec2;
use bevy::prelude::{Gizmos, Query, Resource, Transform, With};
use crate::particle::{get_particle_density, Particle, PredictedPos, Velocity};
use bevy::prelude::*;
use bevy_egui;
...
pub fn particle_velocity_gizmos(mut gizmos: Gizmos, particle_q: Query<(&Velocity, &Transform), With<Particle>>) {
    for (&Velocity(vel), transf) in particle_q.iter() {
        let start = Vec2::new(transf.translation.x, transf.translation.y);
        gizmos.arrow_2d(start, Vec2::new(vel.x + start.x, vel.y + start.y), ORANGE_400);
    }
}

pub fn local_density_gizmos(mut gizmos: Gizmos, particles: Query<&Transform, With<Particle>>, entity_lookup_chunk: Res<EntityLookupChunk>, mouse_pos: Res<MousePosition>) {
    let density = get_particle_density(mouse_pos.0, &particles, &entity_lookup_chunk).unwrap_or_default();
...
pub fn density_grid(mut gizmos: Gizmos, particles: Query<&Transform, With<Particle>>, entity_lookup_chunk: Res<EntityLookupChunk>) {
    let mut x = 0.0;
    let mut y = 0.0;
    ...
    while x <= CHUNK_SIZE as f32 {
        while y <= CHUNK_SIZE as f32 {
            let x_2 = x - 0.5;
            let y_2 = y - 0.5;
            let d = 1.0 - 1.0 / (get_particle_density(Vec2::new(x_2,y_2), &particles, &entity_lookup_chunk).unwrap_or_default() * 10.0).max(0.01);
            let color = Srgba::rgba_u8((255.0 * d) as u8, (200.0 * d) as u8, (255.0 * d) as u8, 100);
            //println!("XY, {x}|{y}");
...
pub fn highlight_neighborhood_entities(mut gizmos: Gizmos, particles: Query<&Transform, With<Particle>>, entity_lookup_chunk: Res<EntityLookupChunk>, mouse_pos: Res<MousePosition>) {
    let chunk_pos = Chunk::<Vec<Entity>>::get_chunk_pos(mouse_pos.0.x, mouse_pos.0.y);
    if chunk_pos.is_none() {return;}
    let chunk_pos = chunk_pos.unwrap();

    let entities = entity_lookup_chunk.get_neighborhood_entities(chunk_pos.x as usize, chunk_pos.y as usize);
    for entity in entities.iter() {
        if let Ok(transform) = particles.get(*entity) {
            gizmos.circle_2d(Vec2::new(transform.translation.x, transform.translation.y),0.05, RED_500);
        }
    }
...
pub fn debug_config_ui(mut contexts: EguiContexts, mut config: ResMut<DebugConfig>, mouse_pos: Res<MousePosition>) {
    ...
    egui::Window::new("Debug Config").show(contexts.ctx_mut(), |ui| {
        ...
        ui.checkbox(&mut config.show_mouse_pos, if show_mouse_pos {
            format!("Mouse Position: {:.1}|{:.1}", mouse_pos.0.x, mouse_pos.0.y)
        } else {
            "Show Mouse Position".to_string()
        });
        ui.checkbox(&mut config.enable_local_density_gizmo, "Local Density Gizmo");
        ui.checkbox(&mut config.highlight_neighborhood_entities, "Highlight Neighborhood Entities");
